...
static const char k_html[] =
  "<!doctype html><html lang='ru'><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1'><title>Hermes StickS3</title><style>*{box-sizing:border-box}body{font-family:system-ui,sans-serif;background:#0b1220;color:#e5e7eb;margin:0;padding:10px}h1{font-size:18px;margin:0 0 10px}.card{background:#111827;border:1px solid #1f2937;border-radius:10px;padding:14px;margin-bottom:10px}label{display:block;font-size:12px;color:#9ca3af;margin:9px 0 3px}input{width:100%;background:#0f172a;color:#e5e7eb;border:1px solid #334155;border-radius:8px;padding:9px;font-size:14px}button{background:#1f2937;color:#e5e7eb;border:1px solid #374151;border-radius:8px;padding:10px 14px;font-size:14px;cursor:pointer}.save{background:#065f46;border-color:#047857;width:100%;margin-top:12px}.row{display:flex;gap:8px;align-items:center}.muted{opacity:.7;font-size:12px}select{flex:1;background:#0f172a;color:#e5e7eb;border:1px solid #334155;border-radius:8px;padding:9px}#wifi-list{display:grid;gap:6px;max-height:300px;overflow:auto;margin:10px 0}.wifi-item{text-align:left;display:flex;flex-direction:column;gap:4px;min-width:0}.wifi-name{overflow-wrap:anywhere}fieldset{min-width:0;border:1px solid #334155;border-radius:8px;margin:12px 0;padding:12px}#wifi-editor{border-top:1px solid #334155;margin-top:12px;padding-top:8px}.row{flex-wrap:wrap}#wifi-error{color:#fca5a5}</style></head><body><h1>🤖 Hermes StickS3 <span class='muted' id='ip'></span></h1><div class='card'><div class='muted'>Сеть и голосовой сервер</div><fieldset><legend>Wi-Fi сети</legend><div class='row'><button type='button' onclick='scanWifi(true)'>Сканировать</button><button type='button' onclick='editWifi()'>Добавить вручную</button></div><div id='wifi-scan-status' class='muted' aria-live='polite'></div><div id='wifi-list' aria-label='Найденные и сохранённые сети'></div><div class='muted'>✓ — сохранена на диктофоне. «Не видна» — не найдена последним сканированием. Изменения применятся после Сохранить и перезагрузить.</div><div id='wifi-editor' hidden><label for='ssid'>Имя сети (SSID)</label><input id='ssid' autocomplete='off'><label for='pass'>Пароль</label><input id='pass' type='password' autocomplete='new-password'><label><input id='wifi-open' type='checkbox' style='width:auto'> Без пароля (открытая сеть)</label><div class='row'><button type='button' onclick='applyWifi()'>Применить</button><button id='wifi-delete' type='button' onclick='removeWifi()'>Удалить сеть</button><button type='button' onclick='closeWifi()'>Отмена</button></div><div id='wifi-error' class='muted' role='alert'></div></div></fieldset><label>Адрес голосового сервера (обязательно)</label><input id='url' placeholder='http://192.168.1.10:8080'><div class='muted' id='gateway-help'>Только базовый адрес, например http://192.168.1.10:8080</div><label id='token-label'>Токен устройства (обязательно)</label><input id='token' type='password' placeholder='Пусто — оставить сохранённый токен'><label for='sleep-seconds'>Переход в сон от батареи (секунды)</label><input id='sleep-seconds' type='number' min='5' max='3600' step='1' value='30'><div class='muted'>От 5 до 3600 секунд бездействия. При питании от USB автоматический сон отключён.</div><fieldset><legend>WireGuard VPN</legend><label><input id='wg-enabled' type='checkbox' style='width:auto'> Включить WireGuard</label><label for='wg-address'>IPv4-адрес диктофона внутри VPN</label><input id='wg-address' type='text' placeholder='10.7.0.2' autocomplete='off'><label for='wg-netmask'>Маска подсети VPN</label><input id='wg-netmask' type='text' placeholder='255.255.255.0' autocomplete='off'><label for='wg-endpoint'>Имя или IPv4-адрес сервера WireGuard</label><input id='wg-endpoint' type='text' placeholder='vpn.example.com' autocomplete='off'><label for='wg-port'>UDP-порт сервера</label><input id='wg-port' type='number' placeholder='51820' autocomplete='off'><label for='wg-public_key'>Публичный ключ сервера</label><input id='wg-public_key' type='text' placeholder='' autocomplete='off'><label for='wg-private_key'>Приватный ключ диктофона</label><input id='wg-private_key' type='password' placeholder='Пусто — оставить сохранённый ключ' autocomplete='off'><label for='wg-preshared_key'>Предварительно согласованный ключ (необязательно)</label><input id='wg-preshared_key' type='password' placeholder='Пусто — оставить сохранённый ключ' autocomplete='off'><label for='wg-keepalive'>Интервал поддержания соединения (секунды; 0 — отключить)</label><input id='wg-keepalive' type='number' placeholder='25' autocomplete='off'><label for='wg-ntp_server'>Сервер времени NTP, доступный без VPN</label><input id='wg-ntp_server' type='text' placeholder='pool.ntp.org' autocomplete='off'><label><input id='wg-clear-psk' type='checkbox' style='width:auto'> Удалить сохранённый предварительно согласованный ключ</label><div class='muted'>При включённом WireGuard голосовой сервер доступен через VPN, в том числе по адресу в домашней сети. VPN также используется для выхода в интернет. Сервер WireGuard должен разрешать доступ к нужной сети. Настройки диктофона остаются доступны по Wi-Fi. Пустые поля ключей сохраняют прежние значения. <span id='wg-keys'></span></div><div class='muted' id='wg-status'></div></fieldset><button class='save' onclick='saveCfg()'>Сохранить и перезагрузить</button><div class='muted' id='info'></div></div><div class='card'><b>Сброс настроек</b><p class='muted'>Удалить все сети Wi-Fi, пароли, настройки сервера и WireGuard. Таймер сна вернётся к 30 секундам.</p><button id='reset-settings' style='color:#ff8a80' onclick='resetCfg()'>Сбросить все настройки</button><div id='reset-status' class='muted' role='status'></div></div><div class='card'><b>Состояние</b><div id='status' class='muted' style='margin-top:6px'>Загрузка…</div></div><script>const $=x=>document.getElementById(x);let savedToken=false;let savedWifi=[],wifiProfiles=[],visibleWifi=[],scanState='loading',editingWifi=-1,wifiLoaded=false;\n"
  "function wgStatusText(status){return ({disabled:'выключен',waiting_wifi:'ожидание Wi-Fi',waiting_time:'синхронизация времени',connecting:'подключение…',connected:'подключён',error:'ошибка подключения',subnet_conflict:'конфликт подсетей'})[status]||'состояние неизвестно';}\n"
  ...
  "}\n"
  "async function load(){try{const j=await (await fetch('/config')).json();savedWifi=j.wifi_networks||[{ssid:j.wifi_ssid||'',password_set:!!j.wifi_password_set}];wifiProfiles=Array.from({length:5},(_,i)=>({ssid:savedWifi[i]?.ssid||'',password:'',open:!!savedWifi[i]?.ssid&&!savedWifi[i]?.password_set}));wifiLoaded=true;$('wifi-editor').hidden=true;renderWifi();$('url').value=j.gateway_url||'';$('sleep-seconds').value=String(j.sleep_timeout_seconds??30);savedToken=!!j.device_token_set;$('wg-enabled').checked=!!j.wg_enabled;$('wg-address').value=String(j.wg_address??\"\");$('wg-netmask').value=String(j.wg_netmask??\"255.255.255.0\");$('wg-endpoint').value=String(j.wg_endpoint??\"\");$('wg-port').value=String(j.wg_port??\"51820\");$('wg-public_key').value=String(j.wg_public_key??\"\");$('wg-keepalive').value=String(j.wg_keepalive??\"25\");$('wg-ntp_server').value=String(j.wg_ntp_server??\"pool.ntp.org\");$('wg-keys').textContent='Приватный ключ: '+(j.wg_private_key_set?'сохранён':'не задан')+'; общий ключ: '+(j.wg_preshared_key_set?'сохранён':'не задан');$('wg-status').textContent='WireGuard: '+wgStatusText(j.wg_status);$('gateway-help').textContent='Только базовый адрес, например http://192.168.1.10:8080';$('ip').textContent=j.ip&&j.ip!=='0.0.0.0'?'· '+j.ip:'';$('status').textContent=j.wifi_connected?'Wi‑Fi подключён · '+(j.active_ssid||'')+' · '+j.ip:'Точка доступа для настройки · '+j.ap_ip;}catch(e){$('status').textContent='Не удалось получить состояние';}}async function saveCfg(){const url=$('url').value.trim();if(!url){$('info').textContent='Введите адрес голосового сервера';return;}if(!url.startsWith('http://')&&!url.startsWith('https://')){$('info').textContent='Адрес сервера должен начинаться с http:// или https://';return;}{if(!$('token').value&&!savedToken){$('info').textContent='Введите токен устройства';return;}try{const parsed=new URL(url);if(parsed.pathname!=='/'||parsed.search||parsed.hash){$('info').textContent='Введите базовый адрес сервера без пути';return;}}catch(e){$('info').textContent='Введите корректный базовый адрес сервера';return;}}if(!wifiLoaded){$('info').textContent='Дождитесь загрузки настроек';return;}if(!$('wifi-editor').hidden&&!applyWifi())return;const networks=wifiProfiles;if(!networks.some(n=>n.ssid)){$('info').textContent='Добавьте хотя бы одну Wi-Fi сеть';return;}const sleep=$('sleep-seconds').value;if(!/^[0-9]+$/.test(sleep)||Number(sleep)<5||Number(sleep)>3600){$('info').textContent='Время до сна должно быть целым числом от 5 до 3600 секунд';return;}const body=new URLSearchParams({gateway_url:url,device_token:$('token').value,sleep_timeout_seconds:sleep});for(let i=0;i<5;i++){body.set('wifi'+i+'_ssid',networks[i].ssid);body.set('wifi'+i+'_password',networks[i].password);body.set('wifi'+i+'_open',networks[i].open?'1':'0');}body.set('wg_enabled',$('wg-enabled').checked?'1':'0');body.set('wg_clear_psk',$('wg-clear-psk').checked?'1':'0');body.set('wg_address',$('wg-address').value.trim());body.set('wg_netmask',$('wg-netmask').value.trim());body.set('wg_endpoint',$('wg-endpoint').value.trim());body.set('wg_port',$('wg-port').value.trim());body.set('wg_public_key',$('wg-public_key').value.trim());body.set('wg_private_key',$('wg-private_key').value.trim());body.set('wg_preshared_key',$('wg-preshared_key').value.trim());body.set('wg_keepalive',$('wg-keepalive').value.trim());body.set('wg_ntp_server',$('wg-ntp_server').value.trim());$('info').textContent='Сохранение…';const r=await fetch('/config',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body});$('info').textContent=r.ok?'Сохранено. Перезагрузка…':'Не удалось сохранить настройки';}async function refreshWg(){try{const j=await(await fetch('/config')).json();$('wg-status').textContent='WireGuard: '+wgStatusText(j.wg_status);$('status').textContent=j.wifi_connected?'Wi‑Fi подключён · '+(j.active_ssid||'')+' · '+j.ip:'Точка доступа для настройки · '+j.ap_ip;}catch(e){}}load();scanWifi();setInterval(refreshWg,3000);</script></body></html>";
...
static esp_err_t h_config_get(httpd_req_t *req) {
  ...
  used += (size_t)snprintf(body + used, sizeof(body) - used,
      ",\"wg_enabled\":%s,\"wg_port\":%u,\"wg_keepalive\":%u,"
  ...
  used += (size_t)snprintf(body + used, sizeof(body) - used,
      ",\"sleep_timeout_seconds\":%lu,\"ip\":\"%s\",\"ap_ip\":\"%s\",\"wifi_connected\":%s,\"wifi_password_set\":%s,\"device_token_set\":%s}",
